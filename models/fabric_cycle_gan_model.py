...
# 把fabric纹理作为额外通道输出
class FabricCycleGANModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        input_nc_ga = 3
        output_nc_ga = 4
        self.netG_A = networks.define_G(input_nc_ga, output_nc_ga, opt.ngf, opt.netG, opt.norm,
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            zero_texture = torch.zeros_like(self.idt_A_texture)
            self.real_A_with_fabric_texture = torch.cat((self.real_A, zero_texture), 1)
            self.idt_B = self.netG_B(self.real_A_with_fabric_texture)
